...
use crate::warp_handlers::get_concrete_items_catalog;
...
async fn main() {
    ...
    let concrete_positions_catalog = warp::path!("catalog" / String)
        .map(|value : String| {
            value.clone()
        })
        .and(warp::get())
    ...
    warp::serve(get_all_positions_catalog.or(concrete_positions_catalog)).run(([0, 0, 0, 0], 8000)).await;
}